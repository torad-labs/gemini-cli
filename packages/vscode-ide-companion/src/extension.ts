  ...
  try {
    ...
    if (
      ...
      const selection = await vscode.window.showInformationMessage(
        `A new version (${latestVersion}) of the torad-code Companion extension is available.`,
...
export async function activate(context: vscode.ExtensionContext) {
  logger = vscode.window.createOutputChannel('torad-code IDE Companion');
  ...
  if (
    ...
    void vscode.window.showInformationMessage(
      'torad-code Companion extension successfully installed.',
    );
    ...
    vscode.commands.registerCommand('gemini-cli.runGeminiCLI', async () => {
      ...
      if (!workspaceFolders || workspaceFolders.length === 0) {
        vscode.window.showInformationMessage(
          'No folder open. Please open a folder to run torad-code.',
        );
        return;
      }
      ...
      if (workspaceFolders.length === 1) {
        ...
        selectedFolder = await vscode.window.showWorkspaceFolderPick({
          placeHolder: 'Select a folder to run torad-code in',
        });
      }
      ...
      if (selectedFolder) {
        ...
        const terminal = vscode.window.createTerminal({
          name: `torad-code (${selectedFolder.name})`,
